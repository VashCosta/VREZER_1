...
public class VrezerAiAgentService {
    ...
    public Map<String, Object> buildDynamicLocalEngineDossier(String resumeText, String jobDescription) {
        ...
        String professionalSummary = String.valueOf(parsed.getOrDefault("professionalSummary", ""));
        if (professionalSummary.isEmpty() || professionalSummary.equals("null") || professionalSummary.length() < 25) {
            professionalSummary = String.valueOf(tempProfile.getOrDefault("professionalSummary", ""));
        }
        if (professionalSummary.isEmpty() || professionalSummary.equals("null") || professionalSummary.length() < 25) {
            professionalSummary = candidateName + " is an aspiring " + targetRole + " with core expertise in "
                + (allSkills.isEmpty() ? domain : String.join(", ", allSkills.subList(0, Math.min(5, allSkills.size()))))
                + ". Demonstrated practical execution across " + domain + " projects and applied system development.";
        }
        result.put("professionalSummary", professionalSummary);
        result.put("secondaryDomain", String.valueOf(tempProfile.getOrDefault("secondaryDomain", "Full Stack Development")));
        result.put("transferableSkills", (softSkills != null && !softSkills.isEmpty()) ? softSkills : List.of("Problem Solving", "Analytical Thinking", "Teamwork", "Quick Learner", "Creative Content"));
        result.put("strategicForecast", "Growth candidate in " + domain + " space with strong foundations in " + (allSkills.isEmpty() ? "modern engineering" : String.join(", ", allSkills.subList(0, Math.min(4, allSkills.size())))) + ".");
